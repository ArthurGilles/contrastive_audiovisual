...
def calculate_accuracy(model, dataloader, device):
    ...
    with torch.no_grad():
        ...
        all_audio_embeddings = all_audio_embeddings.detach().cpu().numpy().astype(np.float32)
        all_visual_embeddings = all_visual_embeddings.detach().cpu().numpy().astype(np.float32)
    ...
    # Calculate accuracy
    audio_accuracy = correct_audio_matches / total_audio_samples if total_audio_samples > 0 else 0.0
    visual_accuracy = correct_visual_matches / total_visual_samples if total_visual_samples > 0 else 0.0
